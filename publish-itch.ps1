...
try {
    if ($needsHtml5Package) {
        ...
        $demoWasm = Join-Path $PSScriptRoot "target-demo\wasm32-unknown-unknown\release\mirexis.wasm"
        ...
        Copy-Item -LiteralPath $demoWasm -Destination (Join-Path $demoDir "mirexis.wasm") -Force
