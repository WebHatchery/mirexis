...
#[test]
fn active_summary_names_each_preparation_effect_and_rejects_unknown_ids() {
    for (id, expected) in [
        ("bastion_mesh", "PREP // BASTION MESH // SQUAD +1 ARM"),
        ("survey_uplink", "PREP // SURVEY UPLINK // SQUAD +8 ACC"),
        ("rapid_injectors", "PREP // RAPID INJECTORS // SQUAD +1 MOV"),
    ] {
        assert_eq!(active_summary(id), Some(expected), "preparation={id}");
    }
