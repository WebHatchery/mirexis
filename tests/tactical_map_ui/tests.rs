use super::*;
use mirexis::first_hour::{FirstHourProgress, FirstHourStage, TacticalLesson};
use mirexis::grid_ui::{STRUCTURE_ART_PIVOT, STRUCTURE_ART_SCALE};
...
fn guided_attack_selects_before_it_commits_the_shot() {
    let data = mirexis::data::GameData::load().unwrap();
    ...
    let hostile = session
        ...
        .units
        .iter_mut()
        .find(|unit| unit.team == mirexis::data::Team::Hostile)
        .unwrap();
...
fn attack_confirmation_card_stays_inside_the_tactical_panel() {
    ...
    let card = mirexis::action_preview_ui::attack_card_bounds(panel);
    ...
    for zoom in [0.65, 1.0, 1.85] {
        for (requested_center, position, edge) in cases {
            ...
            camera.clamp_isometric_with_insets(
                40,
                40,
                mirexis::grid_ui::TACTICAL_HALF_WIDTH,
                mirexis::grid_ui::TACTICAL_HALF_HEIGHT,
                viewport,
...
fn tactical_camera_insets_cover_every_declared_static_sprite() {
    ...
    let tile_width = mirexis::grid_ui::TACTICAL_HALF_WIDTH * 2.0;
    let ground_y = -mirexis::grid_ui::TACTICAL_HALF_HEIGHT
        + mirexis::grid_ui::TACTICAL_HALF_HEIGHT * 2.0 * 0.72;
    for definition in mirexis::visual_assets::VisualCatalog::load()
        .expect("embedded visual catalog loads")
        .units
    {
        let height = tile_width * definition.scale;
        let width = height * (2.0 / 3.0);
        let left = width * definition.pivot[0] - mirexis::grid_ui::TACTICAL_HALF_WIDTH;
        let right = width * (1.0 - definition.pivot[0]) - mirexis::grid_ui::TACTICAL_HALF_WIDTH;
        let top =
            height * definition.pivot[1] - (ground_y + mirexis::grid_ui::TACTICAL_HALF_HEIGHT);
        let bottom = ground_y + height * (1.0 - definition.pivot[1])
            - mirexis::grid_ui::TACTICAL_HALF_HEIGHT;
    ...
    for (scale, pivot) in [
        ...
        let height = width * mirexis::grid_ui::TERRAIN_ART_ASPECT;
        assert!(width * pivot[0] - tile_width * 0.5 <= insets.left);
        assert!(width * (1.0 - pivot[0]) - tile_width * 0.5 <= insets.right);
        assert!(height * pivot[1] - mirexis::grid_ui::TACTICAL_HALF_HEIGHT <= insets.top);
        assert!(
            height * (1.0 - pivot[1]) - mirexis::grid_ui::TACTICAL_HALF_HEIGHT <= insets.bottom
        );
    }
}
