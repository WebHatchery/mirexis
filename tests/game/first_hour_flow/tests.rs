...
fn first_hour_teaching_lane_is_attackable_after_the_cover_move() {
    ...
    mirexis::formation::apply(&mut roster, &mission, &data.config, FormationKind::Wedge);
    ...
    let route = session
        ...
        .unwrap();
    assert_eq!(
        mirexis::tactical::path_cost(&route, &session.tactical.terrain_costs),
        15
    );
}
...
fn next_ready_selection_advances_the_first_hour_select_lesson() {
    ...
    let mission = campaign
        .strategy
        .materialize_selected(&data, &campaign.colony);
    let mut session = mirexis::state::GameSession::new(&data.config, &mission, &data.roster);
    let mut progress = mirexis::first_hour::FirstHourProgress {
        stage: FirstHourStage::FirstOperation,
        lesson: mirexis::first_hour::TacticalLesson::Select,
        ..mirexis::first_hour::FirstHourProgress::default()
    };
    ...
    assert_eq!(
        progress.lesson,
        mirexis::first_hour::TacticalLesson::MoveToCover
    );
}
