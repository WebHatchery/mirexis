...
fn invalid_controller_target_does_not_cancel_targeting() {
    ...
    let session = mirexis::state::GameSession::new(&data.config, &data.mission, &roster);
    let empty_tile = session
        ...
        .unwrap();
    let targeting = mirexis::game::TacticalTargeting::Equipment {
...
fn valid_controller_target_keeps_the_equipment_intent() {
    ...
    let campaign = mirexis::campaign::CampaignState::new(&data);
    let roster = campaign.deployment_roster(&data, &data.mission);
    let mut session = mirexis::state::GameSession::new(&data.config, &data.mission, &roster);
