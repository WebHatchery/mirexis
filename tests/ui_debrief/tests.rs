use super::*;
use mirexis::state::{CharacterConsequence, ObjectiveState};
...
fn demo_completion_changes_the_debrief_exit_label() {
    let data = mirexis::data::GameData::load().unwrap();
    let mut campaign = CampaignState::new(&data);
    campaign.operations_completed = mirexis::demo::MISSION_LIMIT - 1;
    assert_eq!(return_button_label(&campaign), "RETURN TO COLONY");

    campaign.operations_completed = mirexis::demo::MISSION_LIMIT;
    assert_eq!(
        return_button_label(&campaign),
        if mirexis::demo::is_demo_build() {
            "FINISH DEMO"
        } else {
            "RETURN TO COLONY"
        }
