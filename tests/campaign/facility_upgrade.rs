...
fn hot_core_adds_attention_to_the_most_visible_faction_after_an_operation() {
    ...
    let plant_id = campaign
        ...
        .buildings
        .iter()
        .find(|building| building.kind == mirexis::colony::BuildingKind::PowerPlant)
        .unwrap()
        .id
        .clone();
    campaign
        .colony
        .queue_facility_upgrade(&plant_id, mirexis::colony::HOT_CORE_UPGRADE)
        .unwrap();
    campaign.colony.advance_operation();
...
fn drone_bay_reduces_repairs_while_the_workshop_is_online() {
    ...
    let repair = campaign.colony.repair_building("hydroponics").unwrap();
    assert_eq!(repair.materials_spent, 15);
    assert!(campaign
        .colony
        .buildings
        .iter()
