...
fn required_first_hour_dialogues_focus_their_continue_control() {
    ...
    for (progress, character_id) in [(&mara, "mara_venn"), (&ilya, "ilya_reed")] {
        ...
        assert_eq!(
            target.0,
            mirexis::colony_exploration::dialogue_continue_button_bounds()
        );
...
fn dialogue_continue_button_stays_inside_the_conversation_panel() {
    let button = mirexis::colony_exploration::dialogue_continue_button_bounds();
...
fn briefing_focus_moves_to_deploy_for_a_ready_squad() {
    let data = mirexis::data::GameData::load().expect("embedded data loads");
    let mut campaign = mirexis::campaign::CampaignState::new(&data);
    campaign.first_hour.stage = FirstHourStage::FirstBriefing;

    assert_eq!(
        briefing_focus_target(&campaign.first_hour, &campaign, &data),
        Some(mirexis::briefing_deployment_ui::deploy_button_bounds())
    );
}
...
fn briefing_focus_recovers_to_a_selectable_row_when_the_squad_is_empty() {
    ...
    let expected = mirexis::briefing_deployment_ui::deployment_row_bounds(campaign.roster.len(), 0);
    assert_eq!(
        briefing_focus_target(&campaign.first_hour, &campaign, &data),
        Some(expected)
    );
