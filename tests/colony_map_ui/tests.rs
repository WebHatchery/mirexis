...
fn first_hour_route_targets_the_current_guided_colonist() {
    ...
    assert_eq!(
        ...
        mirexis::colony_exploration::npc_position(&campaign, "mara_venn")
    );

    campaign.first_hour.acknowledge_colonist("mara_venn");
    assert!(first_hour_destination(&campaign).is_none());

    campaign.first_hour.stage = mirexis::first_hour::FirstHourStage::FirstReturn;
    assert!(first_hour_destination(&campaign).is_none());
    campaign.first_hour.returned_to_colony();
    assert_eq!(
        first_hour_destination(&campaign),
        mirexis::colony_exploration::npc_position(&campaign, "ilya_reed")
    );
...
#[test]
fn visible_zoom_controls_stay_below_the_interactive_colony_viewport() {
    let panel = panel_bounds(false);
    let viewport = viewport_bounds(panel);
    let origin = zoom_controls_origin(panel);
