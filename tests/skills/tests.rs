...
use mirexis::data::{EdgeDirection, GameData, HazardKind};
use mirexis::state::GameSession;
use mirexis::tactical::{BattleEvent, HazardTile, StatusKind};
...
fn adaptive_secretion_resists_the_nearest_visible_hazard() {
    ...
    let events = mirexis::hazards::resolve_after_move(&mut session, "kira_voss");
...
fn targeted_techniques_explain_team_and_range_failures() {
    ...
    assert_eq!(
        ...
        Err(mirexis::state::RuleError::WrongTeam)
    );
    ...
    assert_eq!(
        ...
        Err(mirexis::state::RuleError::OutOfRange)
    );
}
