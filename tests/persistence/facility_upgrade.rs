...
fn version_177_save_preserves_workshop_upgrade_queue() {
    ...
    campaign
        .colony
        .queue_facility_upgrade(&workshop_id, mirexis::colony::PRECISION_BENCH_UPGRADE)
        .unwrap();
    let session = GameSession::new(&data.config, &data.mission, &data.roster);
    ...
    assert_eq!(
        ...
        mirexis::colony::PRECISION_BENCH_UPGRADE
    );
}
...
#[test]
fn version_103_save_preserves_barracks_upgrade_queue() {
    let data = GameData::load().unwrap();
    let mut campaign = CampaignState::new(&data);
    ...
    campaign
        ...
        .unwrap();
    let session = GameSession::new(&data.config, &data.mission, &data.roster);
    let legacy = serde_json::to_value(session.to_save("1.103.0", &campaign)).unwrap();

    let migrated = migrate_save_value(Some("1.103.0".to_owned()), legacy, &data).unwrap();
