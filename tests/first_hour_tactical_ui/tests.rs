...
use mirexis::data::{GameData, Team};
use mirexis::first_hour::{FirstHourStage, TacticalLesson};
...
fn tactical_focus_tracks_cover_objective_and_hostile_targets() {
    ...
    session
        .tactical
        .cover_edges
        .push(mirexis::data::CoverEdgeDef {
            position: [6, 18],
            direction: mirexis::data::EdgeDirection::North,
            strength: 25,
        });
    ...
    assert!(mirexis::cover_rules::is_cover_position(
...
fn replay_focus_targets_the_visible_skip_control() {
    ...
    assert_eq!(
        replay_focus_target(),
        mirexis::phase_replay::skip_button_bounds()
    );
}
