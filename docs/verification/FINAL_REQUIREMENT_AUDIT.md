...
|---:|---|---|---|---|
| 1 | Audit every rendered screen, overlay, ending, and capture scene | `scripts/capture_ui.ps1`; explicit builders in `src/game/capture_scenes.rs`, `src/game/capture_colony.rs`, `src/game/capture_debrief.rs`, and `src/game/capture_tactical.rs` | Section C of `UI_GRAPHICS_ACCEPTANCE_MATRIX.md` has 85 PASS rows; tests enforce an exact manifest/matrix/inventory set match | PASS |
...
| 4 | Production 2D assets for all recruits and enemy families, facings/states, allegiance, equipment/mutation cues, and portraits | Eleven 4 x 6 unit atlases, recruit/enemy portrait atlases, `src/tactical_unit_ui.rs`, manifest definitions | Root tactical/faction/roster captures; tests resolve every roster record to authored unit and portrait art and reject duplicate or empty atlas cells | PASS |
...
| 7 | One UI system across screens including typography, surfaces, controls, states, portraits, resources, warnings, tooltips, and input cues | `src/ui_widgets.rs`, shared toolkit dark theme/surfaces, portrait and title modules; accurate strategy/tactical controller mappings; visible TITLE, CANCEL TARGET, and SKIP REPLAY touch recovery | `UI_CONTRAST_TYPOGRAPHY_AUDIT.md`; `title`, `title_controller`, `title_hover`; help/log/modals; refreshed targeting and replay scenes; 1024 x 768 strategy-control audit | PASS |
...
| 9 | Validate meaningful checkpoints with Cargo and `publish.ps1` without treating them as sole completion evidence | Test, strict lint, publisher, package, capture, visual, and browser checks are separate gates | Current: 213 gameplay tests plus the source-limit target test; strict Clippy; successful Windows/WebGL publish; native capture and published-browser checks | PASS |
| 10 | Work actively for at least four hours and continue until no acceptance item remains unresolved | The fresh goal timer is the authority; the matrix time row may close only after it exceeds 14,400 active seconds | Authoritative fresh-goal timer recorded 14,406 active seconds before closure | PASS |
...
- Capture integrity: `capture_audit.json` reports 85 expected/actual images, zero missing/extra names, zero size failures, zero magenta diagnostics, zero duplicates, and zero blank/dark/low-detail frames; capture mode fixes the pointer off-canvas except in its explicit hover scene.
...
- Shipping integrity: `publish.ps1` creates Windows and WebGL builds with 27 runtime assets (25.85 MB), includes both manifests and all atlas families, and excludes `art_sources`; SHA-256 checks confirm the deployed `index.html`, `mirexis.wasm`, `assets.zip`, and `mirexis_windows.zip` exactly match their `dist` artifacts.
- Catalog integrity: the root, packaged, and deployed `catalog_thumbnail.png` copies are the same 1280 x 720 title-screen capture with identical SHA-256 digests.
- Native release integrity: all 213 gameplay tests plus the source-limit target also pass under the optimized release profile; the final Windows binary independently rendered title, colony, and tactical smoke captures at 1280 x 720 through the deterministic native harness, and the temporary smoke output was removed after inspection.
- Browser integrity: fresh final preview loads render the WebGL canvas without panic, warning, runtime-error, or WASM-load-error overlays; clicks aimed through HELP leave 1,131,426 compared battlefield bytes unchanged; a tap-only 185%-zoom/pan → TITLE → NEW OPERATION flow restores 1,162,182 colony-world bytes; SKIP REPLAY dismisses an active hostile-phase overlay; a coalesced control-to-drag colony gesture retains 120 materials until `TAP AGAIN TO CONFIRM`, and the equivalent tactical gesture retains 5 AP until the deliberate following tap.
- Office-suite constraint: no LibreOffice/soffice dependency or invocation exists in the worktree or verification workflow.

## Completion

All implementation, capture, audit, test, lint, native, publish, browser, package, and active-time requirements are resolved.
