...
- Browser integrity: a fresh final preview load renders the WebGL canvas at 1200 x 675 with no panic, runtime-error, or WASM-load-error overlay; clicks aimed through the HELP modal at the obscured pan/zoom controls leave 1,131,426 compared battlefield bytes exactly unchanged; a tap-only 185%-zoom/pan → TITLE → NEW OPERATION flow restores 1,162,182 colony-world bytes exactly; SKIP REPLAY dismisses an active hostile-phase overlay by tap.
