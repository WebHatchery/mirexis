...
| memorial_page_two | Memorial Register page two | Long campaign histories could silently truncate after the first six visible rows | `ui_memorial_page_two.png` | PASS |
...
| Final implementation gate | `cargo fmt --package mirexis -- --check`; 521 Mirexis unit tests, source-size gate, and strict Clippy with warnings denied | PASS |
...
| Final full capture | `FINAL_CAPTURE_INVENTORY.md`; 104 PNGs, SHA-256 digests, and machine audit in `capture_audit.json` | PASS |
