...
| Published WebGL rendering remains stable | UI font atlases are populated before the first frame, preventing mid-frame GPU texture replacement | Fresh published session rendered title, colony hover/placement/zoom, briefing/deployment, tactical movement selection, and tactical wheel zoom at a 1280 x 720 browser viewport; no warning, error, panic, or WASM-load failure appeared | PASS |
