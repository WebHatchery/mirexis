...
| Tactical terrain has clearly readable elevation | Replace the small corner-only height bands with multiple meaningful raised and lowered terrain regions across the playable battlefield; cliff faces, top surfaces, silhouettes, and occlusion must communicate height without relying on `H0`/`H1`/`H2` labels | Height-distribution and projection/hit-test regressions plus refreshed default and panned tactical captures showing unmistakable elevation changes | PENDING |
| Tactical tile artwork uses the correct world origin | Review the hard-coded terrain-art rectangle against each tile's projected diamond, ground contact, elevation offset, atlas pivot, and draw/cull bounds; terrain artwork must remain registered to its owning tile while panning and across the full zoom range | Anchor and bounds regression tests plus close, default, and panned root-level captures with outlines temporarily inspected against artwork | PENDING |
