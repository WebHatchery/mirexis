...
|---|---|---|
| Touch/click completion | Every required action has a labelled button, map target, speech marker, or direct tap gesture; guided attacks add a visible forecast-card ATTACK confirmation and a NEXT cue on the confirmation itself, while objective guidance focuses SECURE OBJECTIVE once actionable and return goals focus RETURN TO COLONY; no keyboard-only first-hour action | Confirm in two human touch-primary sessions |
...
- 475 Mirexis unit tests pass, including deployment-row affordance coverage for full,
  ...
  objective-interaction and debrief-return focus coverage, and campaign decision resource-gate
