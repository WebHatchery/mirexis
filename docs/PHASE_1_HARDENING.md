...
|---|---|---|
| Touch/click completion | Every required action has a labelled button, map target, speech marker, or direct tap gesture; guided attacks add a visible forecast-card ATTACK confirmation and a NEXT cue on the confirmation itself, while objective guidance focuses SECURE OBJECTIVE once actionable; no keyboard-only first-hour action | Confirm in two human touch-primary sessions |
...
- 473 Mirexis unit tests pass, including deployment-row affordance coverage for full,
  ...
  objective-interaction focus coverage, and campaign decision resource-gate
