...
|---|---|---|
| Touch/click completion | Every required action has a labelled button, map target, speech marker, or direct tap gesture; guided attacks add a visible forecast-card ATTACK confirmation and a NEXT cue on the confirmation itself, while objective guidance focuses SECURE OBJECTIVE once actionable, return goals focus RETURN TO COLONY, and required conversations focus CONTINUE after the correct speaker opens; no keyboard-only first-hour action | Confirm in two human touch-primary sessions |
...
- 477 Mirexis unit tests pass, including deployment-row affordance coverage for full,
  ...
  objective-interaction, debrief-return, and dialogue-continue focus coverage, and campaign decision resource-gate
...
- The canonical set contains 97 audited 1280×720 scenes, including the first-hour
  ...
  and forecast/first-hour layout changes replace their affected images.
