...
Audit baseline: save/content 1.113.0, validated and committed with this matrix
...
| D04 | Operation one teaches the core loop and operation two demonstrates transfer | Ordered `TacticalLesson` transitions, a cover-tile-gated first move, reachable opening firing lane, contextual tactical focus, and unscripted `SecondOperation`; player transfer must still be observed | IMPLEMENTED / HUMAN PENDING |
...
| D10 | Documentation and verification captures match the build | 87-scene manifest, acceptance matrix test, `capture_audit.json`, `ui_first_hour_guide.png`, and `ui_first_hour_tactical.png` | VERIFIED |
...
| Highlight relevant target without blocking inspection | Non-blocking goal banner, Mara marker and route trace, operation control, tactical overlays, and lesson-specific map/command focus | VERIFIED |
...
| Core-rule order | `TacticalLesson`: select, cover move gated by entering a cover tile, attack through the reachable opening firing lane, enemy phase, objective, ability | VERIFIED |
...
| Canonical captures replaced rather than duplicated | Flat 87-scene capture inventory and audit | VERIFIED |
