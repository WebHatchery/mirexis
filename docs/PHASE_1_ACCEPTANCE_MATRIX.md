...
Audit baseline: save/content 1.112.0, validated and committed with this matrix
...
| D04 | Operation one teaches the core loop and operation two demonstrates transfer | Ordered `TacticalLesson` transitions, a cover-tile-gated first move, contextual tactical focus, and unscripted `SecondOperation`; player transfer must still be observed | IMPLEMENTED / HUMAN PENDING |
...
| Core-rule order | `TacticalLesson`: select, cover move gated by entering a cover tile, attack, enemy phase, objective, ability | VERIFIED |
