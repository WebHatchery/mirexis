...
Audit baseline: save/content 1.224.0, validated and committed with this matrix
...
| D05 | City supplies direction, preparation, character connection, and visible consequences | `first_hour_ui.rs`, `first_hour_investment_ui.rs`, colony conversations plus the persistent touch-visible Field Notes archive and prerequisite-aware Operations treatment, research, and character-event controls, separated first-hour briefing and character-event lanes, outcome-sensitive `first_hour_consequences_ui.rs`, clear debrief goal/result layering, fielded-versus-reserve squad disclosure including recruited fielded colonists, recruited-roster list coverage, deployed-squad XP disclosure, and colony/debrief captures | VERIFIED |
...
| D08 | Audio/settings exist and critical information remains visual | `audio.rs`, audio tests, distinct phase/reinforcement response cues, settings UI with bounded volume meter, settings-modal input isolation, volume/unmute preview feedback, event prose/markers, and browser persistence pass; fatigue remains an observation target | VERIFIED / HUMAN PENDING |
...
| D10 | Documentation and verification captures match the build | 105-scene manifest, scene-local capture reset, progression Operations-surface references for Contact, Research, Adaptation, Escalation, and Mirexis, Contact event Operations reference, Field Notes archive and paginated Memorial Register references including failed-objective history, persistent touch-visible roster equipment readout, acceptance matrix test, `capture_audit.json`, recruited-roster briefing/roster/Gene Lab captures, `ui_settings.png`, `ui_field_notes.png`, `ui_memorial.png`, `ui_memorial_page_two.png`, `ui_first_hour_guide.png`, `ui_first_hour_return.png`, `ui_first_hour_dialogue.png`, `ui_first_hour_promise.png`, `ui_first_hour_ability.png`, `ui_second_operation_tactical.png`, `ui_first_hour_operations.png`, `ui_first_hour_tactical.png`, `ui_first_hour_attack.png`, `ui_first_hour_enemy_phase.png`, `ui_first_hour_objective.png`, and `ui_first_hour_replay.png` | VERIFIED |
| D11 | Parameterless publisher passes and changes use project commit conventions | Recorded in `PHASE_1_HARDENING.md`; rerun and commit required after each audit fix | VERIFIED |
...
| Canonical captures replaced rather than duplicated | Flat 103-scene capture inventory and audit | VERIFIED |
