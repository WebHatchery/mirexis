...
Audit baseline: save/content 1.214.0, validated and committed with this matrix
...
| D08 | Audio/settings exist and critical information remains visual | `audio.rs`, audio tests, distinct phase/reinforcement response cues, settings UI, settings-modal input isolation, volume/unmute preview feedback, event prose/markers, and browser persistence pass; fatigue remains an observation target | VERIFIED / HUMAN PENDING |
...
| D10 | Documentation and verification captures match the build | 100-scene manifest, scene-local capture reset, progression Operations-surface references for Contact, Research, Adaptation, Escalation, and Mirexis, Contact event Operations reference, acceptance matrix test, `capture_audit.json`, recruited-roster briefing/roster/Gene Lab captures, `ui_first_hour_guide.png`, `ui_first_hour_return.png`, `ui_first_hour_dialogue.png`, `ui_first_hour_promise.png`, `ui_first_hour_ability.png`, `ui_second_operation_tactical.png`, `ui_first_hour_operations.png`, `ui_first_hour_tactical.png`, `ui_first_hour_attack.png`, `ui_first_hour_enemy_phase.png`, `ui_first_hour_objective.png`, and `ui_first_hour_replay.png` | VERIFIED |
...
| Canonical captures replaced rather than duplicated | Flat 100-scene capture inventory and audit | VERIFIED |
